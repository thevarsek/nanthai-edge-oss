...
test("finalization publishes once, records fallback use, and removes private candidates", async () => {
  ...
  const result = await finalizePresentationFanoutHandler(ctx, { runId: "run_1" as never });
  ...
  const duplicate = await finalizePresentationFanoutHandler(ctx, { runId: "run_1" as never });
