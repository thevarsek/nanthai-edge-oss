...
import {
  ADVISOR_WATCHDOG_DELAY_MS,
  MAX_ADVISOR_BRIEF_CHARS,
  MAX_ADVISORS_PER_CHAT,
} from "./constants";
import {
...
export async function createAdvisorBatchForTurn(
  ...
  const existingBatch = await ctx.db
    ...
    .withIndex("by_user_message", (query) => query.eq("userMessageId", args.userMessageId))
    .first();
  ...
  if (inheritedAssignments.length === 0 && (args.selections?.length ?? 0) === 0) {
    if (args.brief?.trim()) {
      throw new ConvexError({ code: "ADVISOR_REQUIRED", message: "Choose at least one Advisor for this brief." });
    }
    return null;
  }
  const eligibility = await resolveAdvisorEligibility(ctx, {
    ...
    keptPersonaIds: inheritedAssignments.map((assignment) => assignment.personaId),
    selectedPersonaIds: args.selections?.map((selection) => selection.personaId),
  ...
  if (!eligibility.isAvailable) {
    if ((args.selections?.length ?? 0) === 0 && !args.brief?.trim()) return null;
    throw eligibilityError(eligibility.reasonCode);
  }

  const effective = mergeEffectiveAdvisors(inheritedAssignments, args.selections);
  ...
  const prepared = await Promise.all(effective.map(async (advisor) => {
    const persona = await ctx.db.get(advisor.personaId);
    if (!persona || persona.userId !== args.userId) {
      return { personaId: String(advisor.personaId), owned: false, resolved: null };
    }
    ...
    const availability = await resolveAdvisorModelAvailability(ctx, model.modelId);
    ...
    return {
      ...
      owned: true,
      resolved: {
        advisor: { ...advisor, allowWebSearch: advisor.allowWebSearch || model.legacyOnline },
        persona,
  ...
  const resolved = prepared.flatMap((entry) => entry.resolved ? [entry.resolved] : []);
  ...
  for (const entry of resolved) {
    const runId = await ctx.db.insert("advisorRuns", {
      batchId,
      userId: args.userId,
      chatId: args.chat._id,
      userMessageId: args.userMessageId,
    ...
    const watchdogScheduledFunctionId = await ctx.scheduler.runAfter(
      ADVISOR_WATCHDOG_DELAY_MS,
      internal.advisors.mutations_internal.timeoutRun,
      { runId },
    );
    await ctx.db.patch(runId, { scheduledFunctionId, watchdogScheduledFunctionId });
  }
  return batchId;
}
...
function mergeEffectiveAdvisors(
  ...
  const byPersona = new Map(ordered.map((advisor) => [String(advisor.personaId), advisor]));
  ...
  return ordered.sort((left, right) => left.sortOrder - right.sortOrder)
...
async function persistKeptSelections(
  ...
  const byPersona = new Map(assignments.map((assignment) => [String(assignment.personaId), assignment]));
  let nextSortOrder = assignments.reduce(
    (maximum, assignment) => Math.max(maximum, assignment.sortOrder + 1),
    0,
  );
  let remainingCapacity = Math.max(0, MAX_ADVISORS_PER_CHAT - assignments.length);
  for (const selection of args.selections ?? []) {
    if (!selection.keepAvailable || !availablePersonaIds.has(String(selection.personaId))) continue;
    const existing = byPersona.get(String(selection.personaId));
    if (existing) {
      await ctx.db.patch(existing._id, { allowWebSearch: selection.allowWebSearch, updatedAt: now });
      continue;
    }
...
function validateSelections(selections: AdvisorSelectionInput[] | undefined): void {
  ...
  if (new Set(ids).size !== ids.length || ids.length > MAX_ADVISORS_PER_CHAT) {
    throw new ConvexError({ code: "ADVISOR_LIMIT", message: "Choose up to three distinct Advisors." });
  }
}
