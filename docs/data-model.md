...
| `generatedFiles` | AI-generated file metadata (M10, document-linked in M33) | userId, chatId, messageId, storageId, filename, mimeType, sizeBytes, toolName, documentId?, documentVersionId? |
...
| `userSecrets` | Encrypted OpenRouter key storage | userId, encrypted `apiKey` `enc:v2` envelope, secretEnvelopeVersion, secretKeyId, secretMigratedAt, updatedAt. Updated clients use `oauth/openrouter:exchangeAndStore`; `upsertApiKey` is released-client compatibility only. |
...
| **Added `userSecrets` table** | Historical M13 addition. It now stores a user-bound encrypted OpenRouter key plus envelope/key metadata. Updated clients use the server-side exchange action; app-launch key syncing is legacy behavior only. Current indexes are `by_user` and `by_secret_key`. |
...
This cleanup covers the M36 removed standalone skills: `conditions-precedent-checklist`, `credit-agreement-summary`, `release-notes`, `shareholder-agreement-summary`, and `solo-founder-gtm`. The corresponding workflows remain represented by broader document/legal, internal-communications, and GTM skills rather than separate active catalog rows. M40 reuses those parent skills and introduces no data-model change.
